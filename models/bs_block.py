...
class gs_block_with_attention_fixed_weights(nn.Module):
    def __init__(self, feature_dim, embed_dim, policy='mean', gcn=False, attention=True,
        ...
        self.use_fixed_weights = use_fixed_weights  # Controls whether to use fixed weights
        ...
        # If using attention, add attention parameters
        if self.attention:
    ...
    def forward(self, x, Adj):
        # Check whether fixed weights are used
        if self.use_fixed_weights:
            ...
            x_transformed = x  # If not using fixed weights, use raw input features
    ...
    def aggregate(self, x, Adj):
        ...
        if not self.gcn:
            n = len(adj)
            adj = adj - torch.eye(n).to(adj.device)  # Remove self-connections

        # Use attention mechanism for weighted averaging of neighbor features
        if self.policy == 'mean' and self.attention:
            # Compute attention weights for node features and neighbors
